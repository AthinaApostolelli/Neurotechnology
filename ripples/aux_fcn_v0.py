...
def get_predictions_index(predictions,threshold=0.5):
	'''
		[pred_indexes] = get_predictions_index_new(predictions, thershold)\n
		Returns the begining and ending samples of the events above a given threshold\n
		Inputs:
			predictions:	X, array with the continuous output of a model (even the Gt)
			threshold:		float, signal intervals above this value will be considered events
    ...
    # Increase the detection window: look around the first-pass detected ripples in a window of +/- 50 data points to find where the prob drops to 0.5. 
	aux2=np.copy(predictions)
	buffer = 50  # in data points - empirically determined 

	for r in range(len(begin_indexes)):
		data_slice = aux2[begin_indexes[r]-buffer:end_indexes[r]+buffer]
		first_idx = np.where(data_slice >= 0.5)[0][0] + begin_indexes[r] - buffer - 1
		last_idx = np.where(data_slice >= 0.5)[0][-1] + begin_indexes[r] - buffer 
		aux[first_idx:last_idx]=1
