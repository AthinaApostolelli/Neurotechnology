...
% For now, only the klustakwik case is implemented correctly - 
...
addParameter(p,'clusteringformat','klustakwik',@ischar); % clustering format: [Current options: Phy, klustakwik/neurosuite, klustaViewa]
...
addParameter(p,'getWaveforms',false,@islogical); % Get average waveforms?
...
if ~forceReload
    ...
    if buzcode
        ...
        elseif exist(fullfile(clusteringpath_full,'amplifier.xml'),'file')
        % elseif exist(fullfile(clusteringpath_full,[basename, '.xml']),'file')
            % xml = LoadXml(fullfile(clusteringpath_full,[basename, '.xml']));
            xml = LoadXml(fullfile(clusteringpath_full,'amplifier.xml'));
        end
    end
    switch lower(clusteringFormat)
        ...
        case {'klustakwik', 'neurosuite'}
            ...
            clusteringfile = load(fullfile(clusteringpath_full,'amplifier_res.mat'));
            unit_nb = 0;
            ...
            %     end
            %     % cluster_index = cluster_index(2:end); % needed?
            %     nb_clusters = unique(cluster_index);
            %     nb_clusters2 = nb_clusters(nb_clusters > 1);
            %     for i = 1:length(nb_clusters2)
            %         unit_nb = unit_nb +1;
